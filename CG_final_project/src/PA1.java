    ...
    private void renderGL() {
        ...
        try {
			...
			for(int i=0;i<m.faces.size();i++)
			{
				...
				Vector3f n1 = m.normals.get(Math.abs((int) face.normal.x));
				GL11.glNormal3f(n1.x, n1.y, n1.z);
				Vector2f t1 = m.texVerticies.get(Math.abs((int) face.textures.x));
				GL11.glTexCoord2f(t1.x, t1.y);
				Vector3f v1 = m.verticies.get(Math.abs((int) face.vertex.x));
				GL11.glVertex3f(v1.x, v1.y, v1.z);
				
				Vector3f n2 = m.normals.get(Math.abs((int) face.normal.y));
				GL11.glNormal3f(n2.x, n2.y, n2.z);
				Vector2f t2 = m.texVerticies.get(Math.abs((int) face.textures.y));
				GL11.glTexCoord2f(t2.x, t2.y);
				Vector3f v2 = m.verticies.get(Math.abs((int) face.vertex.y));
				GL11.glVertex3f(v2.x, v2.y, v2.z);

				Vector3f n3 = m.normals.get(Math.abs((int) face.normal.z));
				GL11.glNormal3f(n3.x, n3.y, n3.z);
				Vector2f t3 = m.texVerticies.get(Math.abs((int) face.textures.z));
				GL11.glTexCoord2f(t3.x, t3.y);
				Vector3f v3 = m.verticies.get(Math.abs((int) face.vertex.z));
				GL11.glVertex3f(v3.x, v3.y, v3.z);
				
				if(face.vertex.w>0f){
					Vector3f n4 = m.normals.get(Math.abs((int) face.normal.w));
					GL11.glNormal3f(n4.x, n4.y, n4.z);
					Vector2f t4 = m.texVerticies.get(Math.abs((int) face.textures.w));
					GL11.glTexCoord2f(t4.x, t4.y);
					Vector3f v4 = m.verticies.get(Math.abs((int) face.vertex.w));
