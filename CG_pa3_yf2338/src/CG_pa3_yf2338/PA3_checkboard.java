...
public class PA3_checkboard {

    String windowTitle = "Checkboard";
    ...
    private void initShaders() {
        String vertex_shader =
        		...
        		"const vec4 lightDir2 = vec4(0.0, -20.0, 0.0, 1.0);" +
        		...
        		"void main () {" +
        		"vec4 vertex_position = gl_ModelViewMatrix * gl_Vertex;" +//eye_space
        		"vec3 normal = normalize(gl_NormalMatrix * gl_Normal);" +//unit normal vector
        		"world_vertex = gl_Vertex.xyz;" +
        		...
        		"vec3 light_vert2 = normalize(vec3(light_position2 - vertex_position));" +//incident direction unit vector
        		"vec3 light_refl2 = normalize(reflect(-light_vert2, normal));" +//reflection direction unit vector
				...
				"specular1 = pow(specAngle1, 4.0);" +
        		
        		"if(mode == 2)  specular1 = pow(specAngle1, 16.0);" +
        		"if(mode == 3) specular1 *= lambertian1;" +
        		"if(mode == 4) specular1 *= 0.0;" +
        		"}"+
        		
				"if(lambertian2 > 0.0) {" +
				"vec3 halfDir2 = normalize(light_vert2 + view_dir);"+
				"float specAngle2 = max(dot(halfDir2, normal), 0.0);" +
				"specular2 = pow(specAngle2, 4.0);" +

				"if(mode == 2)  specular2 = pow(specAngle2, 16.0);" +
				"if(mode == 3) specular2 *= lambertian2;" +
				"if(mode == 4) specular2 *= 0.0;" +
				"}"+
        ...
        String fragment_shader =
				...
				"const vec3 specColor1 = vec3(1.0, 0.0, 0.0);" +
				"const vec3 specColor2 = vec3(0.0, 0.0, 1.0);" +
        		...
        		"vec3 color;" +
        		"int count = 0;" +
        		"if (mod(world_vertex.x,0.5)>0.25) count++;" +
        		"if (mod(world_vertex.y,0.5)>0.25) count++;" +
        		"if (mod(world_vertex.z,0.5)>0.25) count++;" +
        		"if (count == 1 || count ==3){" +
